...
prompt = (
        "Create a list of all main topics covered in this non-disclosure agreement (nda), describe each topic with one concise sentence."
        "Focus on short, clear descriptions. "
        "Example: '1. Topic Name: Short Description.', '2. Topic Name: Short Description.'"
    )
...
items = re.split(r'\s(?=\d+\.\s)', combined_summaries_text)
...
parts = re.split(r'\s(?=\d+\.\s)', combined_summaries_text)

# Processing the split parts to remove leading numbers and trimming spaces
cleaned_parts = [part.strip() for part in parts]

# Print each part on a new line for clarity
...
def remove_duplicates(combined_summaries_text):
    # Splitting the combined text into individual summaries based on periods as delimiters
    summaries = sent_tokenize(combined_summaries_text)
    seen = set()
    unique_summaries = []
    for summary in summaries:
        normalized_summary = normalize_text(summary)
        if normalized_summary not in seen:
            seen.add(normalized_summary)
            unique_summaries.append(summary)
    return unique_summaries

# Assuming 'combined_items' is long string of summaries
filtered_summaries = remove_duplicates(cleaned_items)
